    ...
    public partial class Cash_check : Form
    {
        ...
        void txtB_client_phone_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == (char)Keys.Enter)
            {
                ...
                if (MainStaticClass.PassPromo == "")//Бонусная программа выключена
                {
                    // поискать дисконтного клиента по телефону
                    NpgsqlConnection conn = MainStaticClass.NpgsqlConn();
                    try
                    {
                        conn.Open();
                        //string query = "SELECT code FROM clients where phone LIKE'%" + txtB_client_phone.Text.Trim() + "%'";
                        string query = "SELECT code FROM clients where right(phone,10)='" + txtB_client_phone.Text.Trim() + "'";
                        NpgsqlCommand command = new NpgsqlCommand(query, conn);
                        object result_query = command.ExecuteScalar();
                        if (result_query != null)
                        {
                            process_client_discount(result_query.ToString());
                        }
                        else
                        {
                ...
                else //бонусная программа включена и надо поискать клиента
                {
                    ...
                    if (phone.Length != 11)
                    {
                        MessageBox.Show("Номер телефона должен содержать 11 символов, первый символ цифра 7");
        ...
        private void Cash_check_Load(object sender, System.EventArgs e)
        {
            ...
            if (itsnew)
            {
                ...
                //    conn = MainStaticClass.NpgsqlConn();
                //    conn.Open();
                //    string query = "SELECT COUNT(*) FROM last_unload_date WHERE closing_of_day >='" + DateTime.Now.Date.ToString("yyy-MM-dd HH:mm:ss") + "' ";
                //    NpgsqlCommand command = new NpgsqlCommand(query, conn);
                //    result = Convert.ToInt16(command.ExecuteScalar());
                //    if (result != 0)
                //    {
                //        MessageBox.Show("Дата закрытия дня больше текущей, ввод нового документа невозможен !!!", "Проверки корректности ввода");
                //        conn.Close();
                //        //// conn.Dispose();
                //        this.Close();
                //    }
                ...
                //if (result > 0)//На случай если были ошибки при проверке ввода по дате
                //{
                //    return;
                //}
                ...
                //if (MainStaticClass.Use_Trassir > 0)
                //{
                //    if (MainStaticClass.Use_Trassir==1)
                //    {
                //        if (check_type.SelectedIndex == 0)
                //        {
                //            string s = MainStaticClass.get_string_message_for_trassir("POSNG_RECEIPT_OPEN", numdoc.ToString(), MainStaticClass.Cash_Operator, DateTime.Now.Date.ToString("MM'/'dd'/'yyyy"), DateTime.Now.ToString("HH:mm:ss"), "", "", "", "", "", MainStaticClass.CashDeskNumber.ToString(), "");
                //            MainStaticClass.send_data_trassir(s);
                //        }
                //        else
                //        {
                //            string s = MainStaticClass.get_string_message_for_trassir("POSNG_RECEIPT_RETURN", numdoc.ToString(), MainStaticClass.Cash_Operator, DateTime.Now.Date.ToString("MM'/'dd'/'yyyy"), DateTime.Now.ToString("HH:mm:ss"), "", "", "", "", "", MainStaticClass.CashDeskNumber.ToString(), "");
                //            MainStaticClass.send_data_trassir(s);
                //        }
                //    }
                //}
            ...
            else
            {
                ...
                if ((status == 0) || (status == 1))
                {
                    ...
                    //            if (!this.itc_printed())
                    //            {
                    //                this.sale_cancellation.Visible = true;
                    //            }
                    //        }
                    //    }
                    //}
                ...
                else if (status == 2)
                {
                    itsnew = true;
                    Discount = 0;
                    //this.type_pay.Enabled = true;
                    this.label4.Enabled = true;
                    this.check_type.Enabled = true;
                    this.inputbarcode.Enabled = true;
                    this.client_barcode.Enabled = false;
                    //this.sale_cancellation.Enabled = true;
                    //this.comment.Enabled = false;
                    to_open_the_written_down_document();
        ...
        public bool chect_action(string barcode)
        {
            ...
            try
            {
                conn = MainStaticClass.NpgsqlConn();
                conn.Open();
                //string query = "SELECT COUNT(*) FROM action_header WHERE '" + DateTime.Now.Date.ToString("yyy-MM-dd") + "' between date_started AND date_end AND barcode='" + barcode + "'";
                string query = "";
        ...
        private void timer_Tick(object sender, EventArgs e)
        {
            ...
            //    string query = "SELECT barcode  FROM temp_barcode";
            //    command = new NpgsqlCommand(query, conn);
            //    string result = Convert.ToString(command.ExecuteScalar());
            //    query = "DELETE  FROM temp_barcode";
            //    command = new NpgsqlCommand(query, conn);
            //    command.ExecuteNonQuery();
            //    trans.Commit();
            //    if (result.Length > 0)
            //    {
            //        find_barcode_or_code_in_tovar(result); 
            //    }
            //    conn.Close();
        ...
        {
            //if (this.cash.Text.Length == 0)
            //{
            //    this.cash.Text = "0";
            //}
            //int start = cash.SelectionStart;
            //if (this.cash.Text.Length >= 4)
            //{

            //this.cash.Text = Convert.ToDouble(this.cash.Text.Replace(".",",")).ToString("F", System.Globalization.CultureInfo.CurrentCulture);
            //calculate_rests();

            //cash.SelectionStart = curpos;
            //}
        }
        ...
        //private string get_inventory()
        //{
        //    if (inventory.CheckState == CheckState.Checked)
        //    {
        ...
        //}
            ...
            /// <summary>
            /// процедура записи удаленного документа
            /// </summary>
            /// <returns></returns>
        //public bool write_new_document()//write_new_delete_document()
        //{
        //    bool result = false;
        ...
        //        command.Parameters.AddWithValue("cash", this.calculation_of_the_sum_of_the_document().ToString().Replace(",", "."));
        //        command.Parameters.AddWithValue("remainder", "0");
        ...
        //        command.Parameters.AddWithValue("its_deleted", "1");
        ...
        //        command.Parameters.AddWithValue("bonuses_it_is_written_off", "0");
        //        command.Parameters.AddWithValue("is_sent", "0");
        //        command.Parameters.AddWithValue("cash_money", "0");
        //        command.Parameters.AddWithValue("non_cash_money", "0");
        //        command.Parameters.AddWithValue("sertificate_money", "0");
        ...
        */
        ...
        {            
            ...
            /*ProcessingOfActions pa = new ProcessingOfActions();
            pa.client_code = client.Tag.ToString();
            DataTable[] dt = pa.to_process_actions(create_dt());
             * */
        }
